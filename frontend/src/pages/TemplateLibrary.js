...
export default function TemplateLibrary() {
  ...
  useEffect(() => {
    axios.post(`${API}/video-templates/seed`).catch(() => {}).finally(loadVideoTemplates);
  }, [loadVideoTemplates]);
  ...
  const filteredVideo = videoTemplates.filter(t =>
    !search || t.name?.toLowerCase().includes(search.toLowerCase())
  );

  const closeVideoEditor = () => { setShowVideoEditor(false); setEditingVideoTemplate(null); };

  const isVideoEditorOpen = contentTab === "video" && showVideoEditor;

  return (
    <div className="h-full bg-zinc-950 flex flex-col overflow-hidden">

      {/* Header + Search — hidden when video builder is open (it has its own top bar) */}
      {!isVideoEditorOpen && (
        <>
      ...
      {/* Video builder — takes full height */}
      {isVideoEditorOpen && (
        <div className="flex-1 flex flex-col overflow-hidden">
          <VideoTemplateBuilder
            initial={editingVideoTemplate || {}}
            onSaved={() => { loadVideoTemplates(); closeVideoEditor(); }}
            onBack={closeVideoEditor}
          />
        </div>
      )}

      {/* Content — carousel or video card grid */}
      {!isVideoEditorOpen && (
