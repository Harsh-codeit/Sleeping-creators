...
function PlatformsTab({ client, setClient, clientId }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Instagram OAuth Connect Card */}
      <div className="border border-zinc-800 bg-zinc-900 p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-pink-500 to-orange-400 flex items-center justify-center flex-shrink-0">
              ...
              {igConnected ? (
                <div className="space-y-0.5">
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    <span className="text-xs font-mono text-emerald-400">Connected as @{igUsername}</span>
                    {client.instagram_app_index === 2 && (
                      <span className="text-[10px] font-mono px-1.5 py-0.5 bg-purple-900/50 text-purple-400 border border-purple-800">App 2</span>
                    )}
                  </div>
                  {igExpiresAt && (
                    <div className="text-[10px] font-mono text-zinc-600">
                      Token expires: {new Date(igExpiresAt).toLocaleDateString()}
                    </div>
                  )}
                </div>
              ) : (
                <div className="text-xs font-mono text-zinc-500 mt-0.5">Not connected — posts will not publish to Instagram</div>
              )}
            </div>
          </div>
          ...
          {igConnected ? (
            <div className="flex items-center gap-2 flex-shrink-0">
              <button
                data-testid="ig-reconnect-btn"
                onClick={() => connectInstagram(client.instagram_app_index || 1)}
                disabled={connecting}
              ...
              <button
                data-testid="ig-disconnect-btn"
                onClick={disconnectInstagram}
                disabled={disconnecting}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-red-900 text-red-400 hover:bg-red-950 transition-colors duration-150 disabled:opacity-50"
              >
                <Link2Off size={11} />
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <button
                ...
                onClick={() => connectInstagram(1)}
                disabled={connecting}
                className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-pink-600 to-orange-500 text-white text-sm font-semibold hover:opacity-90 transition-opacity duration-150 disabled:opacity-50"
              >
                <Link size={13} />
                {connecting ? "Opening..." : "Connect Instagram"}
              </button>
              <button
                data-testid="ig-connect-btn-2"
                onClick={() => connectInstagram(2)}
                disabled={connecting}
                className="flex items-center gap-2 px-4 py-2 border border-purple-700 text-purple-400 text-sm font-semibold hover:bg-purple-900/30 transition-colors duration-150 disabled:opacity-50"
              >
                <Link size={13} />
                {connecting ? "Opening..." : "Connect Instagram 2"}
              </button>
            </div>
          )}
        </div>
            ...
            <div>
              ...
              {fbConnected ? (
                <div className="space-y-0.5">
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 inline-block" />
                    <span className="text-xs font-mono text-emerald-400">Connected — {fbPageName}</span>
                  </div>
                  {fbExpiresAt && (
                    <div className="text-[10px] font-mono text-zinc-600">
                      Token expires: {new Date(fbExpiresAt).toLocaleDateString()}
                    </div>
                  )}
                </div>
              ) : (
                <div className="text-xs font-mono text-zinc-500 mt-0.5">Not connected — posts will not publish to Facebook</div>
              )}
            </div>
          </div>
          ...
          {fbConnected ? (
            <div className="flex items-center gap-2 flex-shrink-0">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-zinc-700 text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors duration-150 disabled:opacity-50"
              >
                <RefreshCw size={11} className={fbConnecting ? "animate-spin" : ""} />
                Reconnect
              </button>
              <button
            ...
            <button
              onClick={connectFacebook}
              disabled={fbConnecting}
              className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition-colors duration-150 disabled:opacity-50 flex-shrink-0"
            >
              <Link size={13} />
              {fbConnecting ? "Opening..." : "Connect Facebook"}
            </button>
          )}
        </div>

        {/* Page selector — shown when multiple pages are available */}
        {showPageSelector && fbPages.length > 0 && (
          <div className="mt-4 pt-4 border-t border-zinc-800">
            <div className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest mb-3">Select a Page to publish to</div>
            <div className="space-y-2">
              {fbPages.map(page => (
                <div key={page.id} className="flex items-center justify-between p-3 bg-zinc-950 border border-zinc-800 hover:border-zinc-700 transition-colors">
                  <div className="flex items-center gap-3">
                    {page.picture ? (
                      ...
                      <div className="w-8 h-8 bg-blue-900 flex items-center justify-center rounded-full">
                        <span className="text-[10px] text-blue-300 font-bold">{page.name[0]}</span>
                      </div>
                    )}
                    <div>
                      <div className="text-sm text-white">{page.name}</div>
                      {page.category && <div className="text-[10px] font-mono text-zinc-500">{page.category}</div>}
                    </div>
                  </div>
                  <button
                    onClick={() => selectFbPage(page.id)}
                    disabled={selectingPage}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-blue-600 text-white hover:bg-blue-700 transition-colors disabled:opacity-50"
                  >
                    <Check size={11} />
                    Select
                  </button>
