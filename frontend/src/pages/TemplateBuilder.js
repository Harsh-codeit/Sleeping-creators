...
export default function TemplateBuilder() {
  ...
  const [builderTab, setBuilderTab] = useState("carousel"); // "carousel" | "video"
  ...
  useEffect(() => {
    ...
    load();
  }, [id, isClone, navigate, setElements]);
    ...
    <div className="h-full bg-zinc-950 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="px-4 pt-3">
          <div className="flex items-center border border-zinc-800 w-fit mb-3">
            {[["carousel", "Carousel Builder"], ["video", "Video Template"]].map(([val, label]) => (
              <button
                key={val}
                onClick={() => setBuilderTab(val)}
                className={`px-4 py-2 text-xs font-mono uppercase border-r border-zinc-800 last:border-0 ${builderTab === val ? "bg-white text-black font-semibold" : "text-zinc-500 hover:text-white hover:bg-zinc-800"}`}
              >
                {label}
              </button>
            ))}
